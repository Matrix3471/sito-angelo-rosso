  ...
  function doGet(e) {
    try {
      ...
      if (data.type === "newsletter") {
        ...
        // Tab Richieste (chatbot prenotazioni — comportamento esistente)
        var sheet = ss.getSheetByName("Richieste");
        if (!sheet) {
          sheet = ss.insertSheet("Richieste");
          sheet.appendRow(["data","nome","paziente","servizio","citta","relazione","eta","motivo","prescrizione","giorno
  ","orario","note","telefono","email","consenso"]);
        }
        sheet.appendRow([
